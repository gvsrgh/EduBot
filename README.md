...
| 12 | Conversation History | ✅ | Messages saved to DB per chat. **LangGraph PostgreSQL checkpoints** (`PostgresSaver`) now persist full agent state across server restarts. |
...
7. **~~LangGraph Session Checkpoints~~** (`#12`) — *Chandu* ✅
   - Replaced `MemorySaver` with `PostgresSaver` from `langgraph-checkpoint-postgres`
   - Checkpoint tables auto-created on startup via `checkpointer.setup()`
   - Full agent conversation context now persists across server restarts
   - Async `ainvoke` used in all chat routes
